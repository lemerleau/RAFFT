...
def read_true_struct(infile="benchmark_cleaned.csv"):
    results = {}
...
def test_one_seq(record):
    ...
    for istruct, iscore in zip(range(len(conf))[::2], range(len(conf))[1::2]):
        ...
        subprocess.Popen(pred_cmd_line, stdout=subprocess.PIPE, env={'DATAPATH': '../RNAstructure/data_tables/'}).communicate()
        pred_pvv, pred_sens = read_log_file(f"./log/{name}_pred.log")
        results += [(pred_pvv, pred_sens, struct)]
        if pred_pvv >= max_pvv:
            max_pvv, max_sens, max_struct = pred_pvv, pred_sens, struct
    
    mean_pvv = mean([el[0] for el in results])
    mean_sens = mean([el[1] for el in results])
    return mean_pvv, mean_sens, max_struct, seq, name
